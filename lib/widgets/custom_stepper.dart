...
// This is the default material stepper, modified to allow showing content on all steps and disable splash
...
@immutable
class ModifiedControlsDetails {
  /// Creates a set of details describing the Stepper.
  const ModifiedControlsDetails({
...
typedef ModifiedControlsWidgetBuilder =
    Widget Function(BuildContext context, ModifiedControlsDetails details);

/// A builder that creates the icon widget for the [ModifiedStep] at [stepIndex], given
/// [stepState].
typedef ModifiedStepIconBuilder =
    Widget? Function(int stepIndex, ModifiedStepState stepState);
...
class ModifiedStep {
  ...
  const ModifiedStep({
    ...
    this.state = ModifiedStepState.indexed,
    this.isActive = false,
    this.label,
    this.stepStyle,
  });
  ...
  final ModifiedStepState state;

  /// Whether or not the step is active. The flag only influences styling.
  final bool isActive;

  /// Only [ModifiedStepperType.horizontal], Optional widget that appears under the [title].
...
class ModifiedStepper extends StatefulWidget {
  ...
  const ModifiedStepper({
    ...
    this.controller,
    this.physics,
    this.type = ModifiedStepperType.vertical,
    this.currentStep = 0,
    this.onStepTapped,
    this.onStepContinue,
    this.onStepCancel,
  ...
  /// [ModifiedStepperType.horizontal], the content of the current step is displayed
  /// underneath as opposed to the [ModifiedStepperType.vertical] case where it is
  /// displayed in-between.
  final ModifiedStepperType type;
  ...
  ///
  /// This callback which takes in a context and a [ModifiedControlsDetails] object, which
  ...
  /// [ModifiedControlsDetails.currentStep] value within the callback can change the text
  ...
  /// {@end-tool}
  final ModifiedControlsWidgetBuilder? controlsBuilder;

  /// The elevation of this stepper's [Material] when [type] is [ModifiedStepperType.horizontal].
  ...
  ///
  /// When overriding icon for [ModifiedStepState.error], please return
  /// a widget whose width and height are 14 pixels or less to avoid overflow.
  ///
  /// If null, the default icons will be used for respective [ModifiedStepState].
  final ModifiedStepIconBuilder? stepIconBuilder;
  ...
  /// The [ModifiedStep.content] will be clipped to this Clip type.
  ///
...
class _ModifiedStepperState extends State<ModifiedStepper>
  ...
  final Map<int, ModifiedStepState> _oldStates = <int, ModifiedStepState>{};
  ...
  Color _connectorColor(bool isActive) {
    ...
    final Color? resolvedConnectorColor = widget.connectorColor?.resolve(
      states,
    );
  ...
  Widget _buildCircleChild(int index, bool oldState) {
    ...
    return switch (state) {
      ModifiedStepState.indexed ||
      ModifiedStepState.disabled => Text('${index + 1}', style: textStyle),
      ModifiedStepState.editing => Icon(
        Icons.edit,
        color: iconColor,
        size: 18.0,
      ),
      ModifiedStepState.complete => Icon(
        Icons.check,
        color: iconColor,
        size: 18.0,
      ),
      ModifiedStepState.error => const Center(
        child: Text('!', style: _kStepStyle),
      ),
    };
  }
        ...
        child: AnimatedContainer(
          ...
          child: Center(
            child: _buildCircleChild(
              index,
              oldState && widget.steps[index].state == ModifiedStepState.error,
            ),
          ),
        ),
      ),
            ...
            child: CustomPaint(
              ...
              child: Align(
                ...
                child: _buildCircleChild(
                  index,
                  oldState &&
                      widget.steps[index].state != ModifiedStepState.error,
                ),
              ),
            ),
          ),
  ...
  Widget _buildIcon(int index) {
    if (widget.steps[index].state != _oldStates[index]) {
      ...
      if (widget.steps[index].state != ModifiedStepState.error) {
  ...
  TextStyle _titleStyle(int index) {
    ...
    switch (widget.steps[index].state) {
      case ModifiedStepState.indexed:
      case ModifiedStepState.editing:
      case ModifiedStepState.complete:
        return textTheme.bodyLarge!;
      case ModifiedStepState.disabled:
        return textTheme.bodyLarge!.copyWith(
          color: _isDark() ? _kDisabledDark : _kDisabledLight,
        );
      case ModifiedStepState.error:
        return textTheme.bodyLarge!.copyWith(
          color: _isDark() ? _kErrorDark : _kErrorLight,
        );
    }
          ...
          Column(
            ...
            children: <Widget>[
              InkWell(
                ...
                onTap:
                    widget.steps[i].state != ModifiedStepState.disabled
                        ? () {
        ...
        InkResponse(
          onTap:
              widget.steps[i].state != ModifiedStepState.disabled
                  ...
                  : null,
          canRequestFocus: widget.steps[i].state != ModifiedStepState.disabled,
          child: Row(
            children: <Widget>[
              SizedBox(
  ...
  Widget build(BuildContext context) {
    ...
    return switch (widget.type) {
      ModifiedStepperType.vertical => _buildVertical(),
      ModifiedStepperType.horizontal => _buildHorizontal(),
    };
  }
}
...
/// This class is used to override the default visual properties of [ModifiedStep] widgets within a [ModifiedStepper].
///
/// To customize the appearance of a [ModifiedStep] create an instance of this class with non-null parameters
...
///
/// {@tool dartpad}
/// An example that uses [StepStyle] to customize the appearance of each [ModifiedStep] in a [ModifiedStepper].
///
...
class StepStyle with Diagnosticable {
  ...
  ///
  /// This property only applies when [ModifiedStepper.type] is [ModifiedStepperType.horizontal].
